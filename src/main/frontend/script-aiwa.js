...
async function getString() {
    ...
    let avmResponse = await web3.avm.contract.decode(
        "string",
        initialResponse
    );
...
async function setString() {
    ...
    document.querySelector("#set_string_button").innerHTML =
        "Loading...";
    document.querySelector("#set_string_button").disabled = true;

    // Get the value of the input box.
    let inputString = document.querySelector("#input_string_textarea")
        .value;
    ...
    const signedTransaction = await aionweb3.eth
        .signTransaction(transaction, account.privateKey)
        .then(
            transactionResponse => (signedCall = transactionResponse)
        );

    // Send the transaction to the network and wait for a response.
    const transactionReceipt = await aionweb3.eth
